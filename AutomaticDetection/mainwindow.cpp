...
//设置奇偶校验
    ...
    ui->comboBox_xiaoy2->setCurrentIndex(1);
//设置停止位
...
{
    int index = ui->tabWidget_caiji->currentIndex();//设变量为表格中出现的内容
...
void MainWindow::f_openserial()//打开指定串口
{
        ...
        if (!modbusDevice)
        {
            ...
            delete modbusDevice;
            modbusDevice = nullptr;//空指针类型的关键字
        }
        ...
        {        statusBar()->showMessage(modbusDevice->errorString(), 5000);    });
           if (!modbusDevice)
           {
    //           ui->btnOpenFault->setText(QString::fromLocal8Bit("打开"));
               statusBar()->showMessage(tr("Could not create Modbus master."), 5000);        return;
           }
//               Settings1 FaultSerCombox =f_faultwidget->settings;
               statusBar()->clearMessage();
               if (modbusDevice->state() != QModbusDevice::ConnectedState)
               {   modbusDevice->setConnectionParameter(QModbusDevice::SerialPortNameParameter,  portName);
                   modbusDevice->setConnectionParameter(QModbusDevice::SerialParityParameter,    uartParity);
                   modbusDevice->setConnectionParameter(QModbusDevice::SerialBaudRateParameter,  uartBaudRate);
                   modbusDevice->setConnectionParameter(QModbusDevice::SerialDataBitsParameter,  uartDataBits);
                   modbusDevice->setConnectionParameter(QModbusDevice::SerialStopBitsParameter,  uartStopbits);
                   statusBar()->showMessage(tr("Connect Success "));
                   ui->btOpen->setStyleSheet("background-color: white;");
...
void MainWindow::f_closeserial()//关闭串口
{
    ...
    modbusDevice->disconnectDevice();
    statusBar()->showMessage(tr("Connect close") + modbusDevice->errorString(), 5000);
    qDebug()<<"close";
}
...
{
  unsigned int    regStartAddrV = 40010;
  unsigned int    regStartAddr  = 40002;
  unsigned int    regStartAddrF = 40006;
  unsigned int    regStartAddr2 = 40008;
...
//    uint32_t        regData1;
  ...
  p_data1=regData[3]>>16;//高
  qDebug()<<p_data1;
  p_data2=regData[3]&0x0ffff;//低
  qDebug()<<p_data2;
  ...
  statusBar()->clearMessage();
//高端
  QModbusDataUnit writeUnit2(QModbusDataUnit::HoldingRegisters,regStartAddr2,1);//发送的数据信息（数据类型 ，起始地址，个数）
  writeUnit2.setValue(0,1);
  modbusDevice->sendWriteRequest(writeUnit2, ADDR); //发送写请求
//电压

  QModbusDataUnit writeUnit3(QModbusDataUnit::HoldingRegisters,regStartAddrV,2);//发送的数据信息（数据类型 ，起始地址，个数）
  writeUnit3.setValue(0,p_data1);
  writeUnit3.setValue(1,p_data2);
//  modbusDevice->sendWriteRequest(writeUnit, ADDR); //发送写请求
 if (auto *reply = modbusDevice->sendWriteRequest(writeUnit3, ADDR)) {
      if (!reply->isFinished()) {
          connect(reply, &QModbusReply::finished, this, [this, reply]() {
              if (reply->error() == QModbusDevice::ProtocolError) {qDebug()<<reply->error();
...
//频率
 ...
 p_data1=regData[4]>>16;//高
 qDebug()<<p_data1;
 p_data2=regData[4]&0x0ffff;//低
 qDebug()<<p_data2;
 QModbusDataUnit writeUnit4(QModbusDataUnit::HoldingRegisters,regStartAddrF,2);//发送的数据信息（数据类型 ，起始地址，个数）
 writeUnit4.setValue(0,p_data1);
 writeUnit4.setValue(1,p_data2);
//  modbusDevice->sendWriteRequest(writeUnit, ADDR); //发送写请求
if (auto *reply = modbusDevice->sendWriteRequest(writeUnit4, ADDR)) {
     if (!reply->isFinished()) {
         connect(reply, &QModbusReply::finished, this, [this, reply]() {
             if (reply->error() == QModbusDevice::ProtocolError) {qDebug()<<reply->error();
                 statusBar()->showMessage(tr("Write response error: %1 (Mobus exception: 0x%2)")
  ...
  p_data1=regData[0]>>16;//高
  qDebug()<<p_data1;
  p_data2=regData[0]&0x0ffff;//低
  qDebug()<<p_data2;
  QModbusDataUnit writeUnit(QModbusDataUnit::HoldingRegisters,regStartAddr,2);//发送的数据信息（数据类型 ，起始地址，个数）
  writeUnit.setValue(0,p_data1);
  writeUnit.setValue(1,p_data2);
//  modbusDevice->sendWriteRequest(writeUnit, ADDR); //发送写请求
  ...
  sleep(3000);
  p_data1=regData[1]>>16;//高
  qDebug()<<p_data1;
  p_data2=regData[1]&0x0ffff;//低
  qDebug()<<p_data2;
  ...
  sleep(1000);
  p_data1=regData[2]>>16;//高
  qDebug()<<p_data1;
  p_data2=regData[2]&0x0ffff;//低
  qDebug()<<p_data2;
  writeUnit.setValue(0,p_data1);
  writeUnit.setValue(1,p_data2);
  if (auto *reply = modbusDevice->sendWriteRequest(writeUnit, ADDR)) {
  ...
  for(int i=0;i<row;i++)
  {
      ...
      sleep(1000);
      ui->tab_FaultDisplay->setItem(i,4,new QTableWidgetItem(a));
      qDebug()<<num3;
      sleep(num3);
  }
